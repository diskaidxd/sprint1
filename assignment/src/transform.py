...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    df1 = orders.merge(items, how='inner',on='order_id')
    data = df1.merge(products, how='inner', on='product_id')
    ...
    aggregations =  delivered.groupby(delivered["order_id"]).agg({'freight_value': 'sum', 'product_weight_g': 'sum'})
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_ammount_per_date["date"] = pd.to_datetime(
        order_purchase_ammount_per_date["date"]
    )
    ...
    result_df["holiday"] = result_df["holiday"] == "both"
    result_df["holiday"].fillna(False, inplace=True)
    result_df = result_df.loc[:, ['date', 'order_count', 'holiday']]
    # Keep the code below as it is, this will return the result from
    # `aggregations` variable with the corresponding name and format.
